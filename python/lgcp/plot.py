...
def subfigurelabel(x, fontsize=10, dx=39, dy=7, ax=None, bold=True, **kwargs):
    ...
    plt.text(plt.xlim()[0] - px2x(dx), plt.ylim()[1] + px2y(dy), x, **fontproperties)


def yscalebar(
    ycenter, yheight, label, x=None, color="k", fontsize=9, ax=None, side="left"
):
...
def xscalebar(xcenter, xlength, label, y=None, color="k", fontsize=9, ax=None):
    ...
    yl = ax.get_ylim()
    xl = ax.get_xlim()
    plt.plot(xspan, [y, y], color="k", lw=1, clip_on=False)
    plt.text(
        np.mean(xspan),
        y - px2y(5),
        label,
...
def inference_summary_plot(
    ...
    if ax is None:
        ...
        subplots_adjust(left=0.0, right=0.95, top=0.8, wspace=0.8)
        ax = {}
        ax[1] = subplot(141)
        ax[2] = subplot(142)
        ax[3] = subplot(143)
    ...
    # vmax = ceil (vmax*10)/10
    plt.imshow(
        μλ.reshape(L, L) * nanmask, vmin=vmin, vmax=vmax, cmap=cmap, extent=extent
    )
    ...
    vmin = np.floor(vmin * 10) / 10
    vmax = ceil(vmax * 10) / 10
    plt.imshow(toshow, vmin=vmin, vmax=vmax, cmap=cmap, extent=extent)
    plt.plot(*data.arena.perimeter.T, lw=2, color="w")
    ...
    sca(ax[4])
    toshow = cv * nanmask
    vmin, vmax = np.nanpercentile(toshow, [0.5, 99.5])
...
def unit_crosshairs(draw_ellipse=True, draw_cross=True):
    ...
    if draw_ellipse:
        ...
        lines += list(circle)
    if draw_cross:
        lines += [np.nan] + list(1.0 * np.linspace(-1, -0.4, 2))
        lines += [np.nan] + list(1j * np.linspace(-1, -0.4, 2))
        lines += [np.nan] + list(1.0 * np.linspace(1, 0.4, 2))
        lines += [np.nan] + list(1j * np.linspace(1, 0.4, 2))
    elif draw_cross == "full":
        lines += [np.nan] + list(1.0 * np.linspace(-1, 1, 10))
...
def covariance_crosshairs(S, p=0.95, draw_ellipse=True, draw_cross=True, mode="2D"):
    """
    ...
    S = np.float32(S)
    if not S.shape == (2, 2):
        raise ValueError(f"S should be a 2x2 covariance matrix, got {S}")
    if mode == "2D":
    ...
    try:
        ...
        raise ValueError(
            f"Could not get covariance eigenspace, is S={repr(S)} full-rank?"
        ) from err
    e = np.maximum(0, e)
...
def good_colorbar(
    ...
    plt.sca(cax)
    plt.imshow(
        np.array([np.linspace(vmax, vmin, 100)]).T,
...
def colored_boxplot(
    ...
    **kwargs:
    ...
    if mediancolor is None:
        try:
            mediancolor = [BLACK if mpl.colors.to_hex(c) != b else WHITE for c in color]
        except Exception:
            mediancolor = BLACK if mpl.colors.to_hex(color) != b else WHITE
    bp = plt.boxplot(
        data,
        positions=positions,
        patch_artist=True,
