...
def chinv(X, dtype=np.float32):  # inv(x)=C.T@C
    ...
    if info != 0:
        raise ValueError(
            "lapack.dtrtri: "
            + (
                f"arg {-info:d} invalid"
                if info < 0
                else f"diagonal element {info:d} is 0"
            )
        )
    return dtype(X)
...
def sdiv(A, B, fill=0.0, eps=1.0842022e-19, inf=1.8446743e19):
...
def blurkernel2D(V, W, H=None, normalize=False):
    if H is None:
        H = W
    V = np.array(V)
    Λ = np.eye(2) / V**2 if np.size(V) == 1 else np.linalg.pinv((V + V.T) * 0.5)
...
def xygrid(
    ...
    g = np.array(
        [gx[None, :] * np.ones((Hr, Wr)), gy[:, None] * np.ones((Hr, Wr))]
    )  # 2HW
    if z:
        return p2c(g)  # HW
...
def ideal_hex_grid(L, P):
    ...
    coords = zgrid(L)
    return np.sum(
        np.float32([np.cos((θ * coords).real * 2 * np.pi / P) for θ in θs]), 0
    )
...
def ndpoints(a, d=2):
    ...
    if np.any(np.iscomplex(a)):
        if d != 2:
            raise ValueError(f"Cannot interpret complexas dimension={d:d} points")
    ...
    if n == 1:
        ...
        if not d == 1:
            raise ValueError(f"Cannot interpret 1D array as {d:d}-d points")
        return a.reshape(1, len(a))
    if np.sum(s == d) < 1:
        raise ValueError(f"Array needs at least one axis dimension {d:d}")
    if np.sum(s == d) > 2:
        raise ValueError(
...
def bin_points(p, shape, w=None, wrap=None, method="linear"):
    ...
    if method == "nearest":
        ...
        raise ValueError('method should be "linear" or "nearest"')
...
def get_edges(signal, pad_edges=True):
    ...
    if tuple(sorted(np.unique(signal))) == (-2, -1):
        raise ValueError(
            "signal should be bool or int∈{0,1};" + " (using ~ on an int array?)"
        )
