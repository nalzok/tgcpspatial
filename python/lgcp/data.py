...
class Arena:
    ...
    def __init__(self, points, shape, margin=0.1, radius=0.0):
        ...
        if not (W - 1) / H <= self.aspect <= (W + 1) / H:
            raise ValueError(
                f"Data have aspect w/h={self.aspect:f} but W/H={W:d}/{H:d}={W / H:f};"
            )
...
class Dataset:
    ...
    def __init__(self, x, y, spikes):
        ...
        if not np.shape(x) == np.shape(y) == np.shape(spikes):
            raise ValueError("(x,y,spikes) should be arrays with the same size.")
        self.xy = (x, y)
        self.spikes = spikes
    ...
    def from_file(fn):
        ...
        if len(spike_times_samples) == 0:
            warnings.warn(
                f"The `spikes_times` variable for file {fn} appears to be empty.",
                stacklevel=2,
            )
        # Convert units
    ...
    def prepare(self, shape=128, P=None, spike_weights=None, **kw):
        self.arena = Arena(self.xy, shape, **kw)
        N, K = self.arena.binto(
            self.xy,
            self.spikes,
            spike_weights,
        )
